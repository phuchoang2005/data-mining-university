...
## 2. Target Encoding với Smoothing cho `cell_type`

> **Cập nhật:** Mặc dù `cell_type` có mối quan hệ mạnh với `anomaly_label` (p*value = 0.0), nhưng với **Target Encoding kèm Smoothing** (alpha=10), chúng ta có thể giảm thiểu overfitting bằng cách làm mịn giá trị mã hóa về phía mean toàn cục. Công thức: $$smoothed = \frac{count \times mean*{category} + \alpha \times mean\_{global}}{count + \alpha}$$

- **Lý do thay đổi:** Target Encoding cho phép mô hình học trực tiếp mối quan hệ cell_type → anomaly, trong khi smoothing ngăn chặn việc ghi nhớ quá mức.
- **Xử lý nhóm hiếm:** Gom nhóm < 2% vào `Other_Rare_Types` trước khi encoding.
- **Lưu ý:** Theo dõi performance trên validation set để đảm bảo không overfitting.
