...
- **Hướng xử lý:**
  ...
  - **`cell_type`:** Sử dụng **Target Encoding với Smoothing** (thay vì One-Hot Encoding). Mặc dù p_value=0.0 cho thấy mối quan hệ mạnh, nhưng với smoothing (alpha=10), chúng ta có thể tránh overfitting bằng cách làm mịn giá trị mã hóa về phía mean toàn cục. Với các nhóm có tỷ lệ < 2% (như `Sickle_Cell`, `Spherocyte`), hãy gom chúng vào một nhóm chung gọi là **"Other_Rare_Types"**.
...
| :---------------------- | :--------------------- | :-------------------------------- | :------------------------------------------------------------ |
| **`cell_id`**           | N/A                    | **Drop**                          | Cột định danh, không có giá trị dự đoán                       |
| **`cell_type`**         | 0.0 (Rất mạnh)         | **Target Encoding với Smoothing** | Gom nhóm các loại hiếm (< 2%). Smoothing để tránh Overfitting |
...
[Chi tiết](./category/detail_Categorical&Numberical.md)
