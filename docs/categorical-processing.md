...
- **Hướng xử lý:**
  ...
  - **`cell_type`:** Sử dụng **One-Hot Encoding** (không dùng Target Encoding vì p_value=0.0 nghĩa là mối quan hệ gần như xác định — Target Encoding sẽ tạo ra bản sao gần hoàn hảo của nhãn mục tiêu, gây Overfitting). Với các nhóm có tỷ lệ < 2% (như `Sickle_Cell`, `Spherocyte`), hãy gom chúng vào một nhóm chung gọi là **"Other_Rare_Types"**.
...
| **`cell_type`**         | 0.0 (Rất mạnh)         | **One-Hot Encoding**  | Gom nhóm các loại hiếm (< 2%). Không dùng Target Encoding (gây Overfitting do p=0.0) |
